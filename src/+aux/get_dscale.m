%% path continuation - aux.get_dscale
%
%   Institute of Dynamics and Vibration Research
%   Leibniz University Hannover
%   06.11.2020 - Alwin Förster
%
function [dscale] = get_dscale(Opt,Path)
    if Opt.scaling.dynamicdscale
        % get latest solution
        x_latest = [Path.var_all(:,end); Path.l_all(end)];
        
        % find new dscale
        if numel(Opt.dscale_min) == 1 || numel(Opt.dscale_min) == numel(Path.var_all(:,end))+1
