...
%
function [fun_predictor,Jac_predictor] = predictor_taylor(Path,no,nf)
    ...
    p_sc = polyfitn((Path.s_all(end+((-ns+1):0))-s_basis)./dsc_s,(x_all(:,end+((-ns+1):0))-x_basis)./kron(dsc_x,ones(1,ns)),no);
    fun_predictor_sc = @(s) polyvaln(p_sc,(Path.s_all(end)+s-s_basis)/dsc_s,nd);
    if nargout>1
        dp_sc = polydern(p_sc,nd);
        Jac_predictor_sc = @(s) polyvaln(dp_sc,(Path.s_all(end)+s-s_basis)/dsc_s,nd);
    end
    %% descale predictor:
